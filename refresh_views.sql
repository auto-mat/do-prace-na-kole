...
CREATE TABLE dpnk_gpxfile_anonymized as select * from (
   select a.id, a.campaign_id, age_group, sex, occupation_id, ST_LineSubstring(a.track, 100/st_length(a.track::geography), 1 - 100/st_length(a.track::geography)) as the_geom from (
      SELECT dpnk_gpxfile.id, dpnk_userattendance.campaign_id, dpnk_userprofile.age_group, dpnk_userprofile.sex, dpnk_userprofile.occupation_id, (ST_Dump(dpnk_gpxfile.track::geometry)).geom AS track from dpnk_gpxfile
      join dpnk_userattendance on (dpnk_gpxfile.user_attendance_id = dpnk_userattendance.id)
...
CREATE TABLE dpnk_trip_anonymized as select * from (
   select a.id, a.campaign_id, age_group, sex, occupation_id, ST_LineSubstring(a.track, 100/st_length(a.track::geography), 1 - 100/st_length(a.track::geography)) as the_geom from (
      SELECT dpnk_trip.id, dpnk_userattendance.campaign_id, dpnk_userprofile.age_group, dpnk_userprofile.sex, dpnk_userprofile.occupation_id, (ST_Dump(dpnk_trip.track::geometry)).geom AS track from dpnk_trip
...
create table dpnk_track_anonymized as select * from (
   select id, campaign_id, st_numpoints((st_dump(track::geometry)).geom) as numpoints, ST_LineSubstring(track::geometry, 100/st_length(track), 1 - 100/st_length(track)) as the_geom from dpnk_userattendance
