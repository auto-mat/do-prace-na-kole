...
class TestEmails(TestCase):
    def setUp(self):
        ...
        self.subsidiary = Subsidiary.objects.create(
            company=self.company, city=self.city
        )
        self.team = Team.objects.create(
            name="Testing team", campaign=self.campaign, subsidiary=self.subsidiary
        )
        ...
        self.user_tm1 = User.objects.create(
            first_name="Team",
            last_name="Member 1",
            username="user2",
            email="user2@email.com",
        )
        self.userprofile_tm1 = UserProfile.objects.create(
            user=self.user_tm1, language="cs"
        )
        ...
        self.user_tm2 = User.objects.create(
            first_name="Team",
            last_name="Member 2",
            username="user3",
            email="user3@email.com",
        )
        self.userprofile_tm2 = UserProfile.objects.create(
            user=self.user_tm2, language="en"
        )
        ...
        self.invoice = Invoice.objects.create(
            company=self.company, campaign=self.campaign
        )
    ...
    def test_send_approval_request_mail(self):
        ...
        self.assertEqual(
            mail.outbox[0].subject, "[Testing campaign 1] Máte nového člena"
        )
        self.assertInHTML(
            "<title>[Testing campaign 1] Máte nového člena</title>",
            mail.outbox[0].alternatives[0][0],
        )
        self.assertEqual(mail.outbox[1].subject, "[Testing campaign 1] New Team Member")
        self.assertInHTML(
            "<title>[Testing campaign 1] New Team Member</title>",
    ...
    def test_send_invitation_register_mail(self):
        email.invitation_mail(
            ...
            self.user_attendance.userprofile.user.email,
            self.user_attendance,
        )
        ...
        if self.userprofile.language == "cs":
            html_message = "<p>Hezký den Teste,</p>"
        else:
            html_message = "<p>Good news Test!</p>"
    ...
    def test_send_register_mail(self):
        ...
        if self.userprofile.language == "cs":
            ...
            self.assertEqual(
                msg.subject, "[Testing campaign 1] Registration Confirmation"
            )
        self.assertEqual(msg.to[0], "user1@email.com")
        link = "https://dpnk.dopracenakole.cz%s/" % (
            language_url_infix(self.userprofile.language)
        )
        self.assertTrue(link in msg.body)
        if self.userprofile.language == "cs":
            ...
            html_message = (
                "<p>Our ride begins at Oct. 20, 2010 and finishes at Nov. 20, 2010.</p>"
            )
        self.assertInHTML(html_message, msg.alternatives[0][0])
        etree.fromstring(msg.alternatives[0][0], self.parser)
    ...
    def test_unfilled_rides_notification(self):
        ...
        if self.userprofile.language == "cs":
            self.assertEqual(
                str(msg.subject), "[Testing campaign 1] Poslední šance doplnit jízdy"
            )
        else:
            self.assertEqual(
                str(msg.subject), "[Testing campaign 1] Last Chance to Get on Track"
            )
        self.assertEqual(msg.to[0], "user1@email.com")
        link = "https://dpnk.dopracenakole.cz%s/" % (
            language_url_infix(self.userprofile.language)
    ...
    def test_send_team_membership_approval_mail(self):
        ...
        msg = mail.outbox[0]
        if self.userprofile.language == "cs":
            self.assertEqual(msg.subject, "[Testing campaign 1] Jste členem týmu")
        else:
            self.assertEqual(msg.subject, "[Testing campaign 1] You're a Team Member")
        self.assertEqual(msg.to[0], "user1@email.com")
        etree.fromstring(msg.alternatives[0][0], self.parser)

    def test_send_team_membership_denial_mail(self):
        email.team_membership_denial_mail(
            self.user_attendance, self.user_attendance, "reason of denial"
        )
        self.assertEqual(len(mail.outbox), 1)
        msg = mail.outbox[0]
        if self.userprofile.language == "cs":
            self.assertEqual(
                msg.subject, "[Testing campaign 1] Nemůžete se přidat k týmu"
            )
        else:
            self.assertEqual(
                msg.subject, "[Testing campaign 1] You Can't Join the Team"
            )
        self.assertEqual(msg.to[0], "user1@email.com")
        link = "https://dpnk.dopracenakole.cz%s/tym/" % (
            language_url_infix(self.userprofile.language)
        )
        self.assertTrue(link in msg.body)
        etree.fromstring(msg.alternatives[0][0], self.parser)
    ...
    def test_send_team_created_mail(self):
        ...
        msg = mail.outbox[0]
        if self.userprofile.language == "cs":
            self.assertEqual(msg.subject, "[Testing campaign 1] Máte nový tým")
        else:
            self.assertEqual(msg.subject, "[Testing campaign 1] Your New Team")
        self.assertEqual(msg.to[0], "user1@email.com")
        link = "https://dpnk.dopracenakole.cz%s/pozvanky/" % (
            language_url_infix(self.userprofile.language)
        )
        self.assertTrue(link in msg.body)
        etree.fromstring(msg.alternatives[0][0], self.parser)
    ...
    def test_send_invitation_mail(self):
        ...
        self.assertTrue(link in msg.body)
        if self.userprofile.language == "cs":
            html_message = "<p>Hezký den,</p>"
        else:
            html_message = "<p>Good news!</p>"
        self.assertInHTML(html_message, msg.alternatives[0][0])
        etree.fromstring(msg.alternatives[0][0], self.parser)
    ...
    def test_send_payment_confirmation_mail(self):
        ...
        msg = mail.outbox[0]
        if self.userprofile.language == "cs":
            self.assertEqual(msg.subject, "[Testing campaign 1] Startovné je uhrazeno")
        else:
            self.assertEqual(
                msg.subject, "[Testing campaign 1] Participation Fee Accepted"
            )
        self.assertEqual(msg.to[0], "user1@email.com")
        etree.fromstring(msg.alternatives[0][0], self.parser)
    ...
    def test_send_payment_confirmation_company_mail(self):
        ...
        msg = mail.outbox[0]
        if self.userprofile.language == "cs":
            self.assertEqual(msg.subject, "[Testing campaign 1] Jste ve hře")
        else:
            self.assertEqual(msg.subject, "[Testing campaign 1] You're on Track")
        self.assertEqual(msg.to[0], "user1@email.com")
        etree.fromstring(msg.alternatives[0][0], self.parser)
    ...
    def test_send_company_admin_register_competitor_mail(self):
        ...
        if self.userprofile.language == "cs":
            self.assertEqual(
                msg.subject,
                "[Testing campaign 1] Potvrzení registrace firemního koordinátora",
            )
        else:
            self.assertEqual(
                msg.subject,
                "[Testing campaign 1] Company Coordinator Registration Accepted",
            )
        self.assertEqual(msg.to[0], "user1@email.com")
        etree.fromstring(msg.alternatives[0][0], self.parser)
    ...
    def test_send_company_admin_approval_mail(self):
        ...
        if self.userprofile.language == "cs":
            self.assertEqual(
                mail.outbox[0].subject, "[Testing campaign 1] Jste firemní koordinátor"
            )
        else:
            self.assertEqual(
                mail.outbox[0].subject,
                "[Testing campaign 1] You Are Company Coordinator",
            )
        self.assertEqual(mail.outbox[0].to[0], "user1@email.com")
        msg = mail.outbox[0]
        link = "https://dpnk.dopracenakole.cz%s/spolecnost/editovat_spolecnost/" % (
            language_url_infix(self.userprofile.language)
        )
        self.assertTrue(link in msg.body)
        if self.userprofile.language == "cs":
    ...
    def test_send_company_admin_rejected_mail(self):
        ...
        if self.userprofile.language == "cs":
            self.assertEqual(
                mail.outbox[0].subject, "[Testing campaign 1] Špatné zprávy"
            )
        else:
            self.assertEqual(mail.outbox[0].subject, "[Testing campaign 1] Sad News")
        self.assertEqual(mail.outbox[0].to[0], "user1@email.com")
        msg = mail.outbox[0]
        if self.userprofile.language == "cs":
    ...
    def test_new_invoice_mail(self):
        ...
        if self.userprofile.language == "cs":
            self.assertEqual(
                mail.outbox[0].subject, "[Testing campaign 1] Nová faktura vystavena"
            )
        else:
            self.assertEqual(
                mail.outbox[0].subject, "[Testing campaign 1] New Invoice Issued"
            )
        self.assertEqual(mail.outbox[0].to[0], "user1@email.com")
        msg = mail.outbox[0]
        if self.userprofile.language == "cs":
    ...
    def test_unpaid_invoice_mail(self):
        ...
        if self.userprofile.language == "cs":
            self.assertEqual(
                mail.outbox[0].subject, "[Testing campaign 1] Nezaplacená faktura"
            )
        else:
            self.assertEqual(
                mail.outbox[0].subject, "[Testing campaign 1] Unpaid Invoice"
            )
        self.assertEqual(mail.outbox[0].to[0], "user1@email.com")
        msg = mail.outbox[0]
        if self.userprofile.language == "cs":
