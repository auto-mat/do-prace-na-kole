...
class MailingTests(DenormMixin, TestCase):
    ...
    def setUp(self):
        ...
        Token.objects.filter(user=self.user_attendance.userprofile.user).update(
    ...
    custom_fields = [
        ...
        OrderedDict(
            (
                ("Key", "Auth_token"),
                ("Value", "d201a3c9e88ecd433fdbbc3a2e451cbd3f80c4ba"),
            )
        ),
    ...
    def test_cm_add(self):
        ...
        createsend.Subscriber.add.assert_called_once_with(
            "12345abcde", "test@test.cz", "Testing User 1", self.custom_fields, True
        )
        self.assertEqual(self.user_attendance.userprofile.mailing_id, ret_mailing_id)
        self.assertEqual(
            self.user_attendance.userprofile.mailing_hash,
            "0ef7bf8842d0f25db6a7e108b8d46228",
        )
    ...
    def test_cm_update(self):
        ...
        createsend.Subscriber.update.assert_called_once_with(
            "test@test.cz", "Testing User 1", custom_fields, True
        )
        self.assertEqual(
            self.user_attendance.userprofile.mailing_hash,
            "1e57792f94b9a95f1d6c812429873b40",
        )
    ...
    def test_eco_add(self):
        ...
        expected_dict = (
            mailing.TOKENAPI().lists().subscribe.post.call_args_list[0][0][0]
        )
        ...
        mailing.add_or_update_user_synchronous(self.user_attendance)
        self.assertFalse(
            getattr(mailing.TOKENAPI().lists(), "update-subscriber").put.called
        )
        self.assertEqual(
            self.user_attendance.userprofile.mailing_hash,
            "0ef7bf8842d0f25db6a7e108b8d46228",
    ...
    def test_eco_update(self):
        ...
        self.user_attendance.team.subsidiary.save()
        mailing.TOKENAPI = mock.Mock()
        mailing.TOKENAPI().lists().__call__("update-subscriber").put.return_value = {
            "id": ret_mailing_id
        }
        mailing.TOKENAPI().lists().unsubscribe._request.return_value = mock.Mock(
            status_code=200
        )

        mailing.add_or_update_user_synchronous(self.user_attendance)
        mailing.TOKENAPI().lists().__call__(
            "update-subscriber"
        ).put.assert_called_once_with(mock.ANY)
        expected_dict = (
            mailing.TOKENAPI()
            .lists()
            .__call__("update-subscriber")
            .put.call_args_list[0][0][0]
        )
        eco_subscriber_data = copy.deepcopy(self.eco_subscriber_data)
        ...
        self.user_attendance.userprofile.user.save()
        mailing.add_or_update_user_synchronous(self.user_attendance)
        mailing.TOKENAPI().lists().unsubscribe._request.assert_called_once_with(
            "DELETE", data={"email": "test@test.cz"}
        )
