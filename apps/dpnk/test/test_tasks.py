...
class TestSendUnpaidInvoiceNotification(TestCase):
    ...
    def test_notification(self):
        ...
        mail.outbox = []
        mail_count = tasks.send_unpaid_invoice_notification(
            campaign_slug=testing_campaign().slug
        )
        self.assertEqual(mail_count, 1)
        self.assertEqual(len(mail.outbox), 1)
        msg = mail.outbox[0]
        self.assertEqual(msg.recipients(), ["test@test.cz"])
        self.assertEqual(
            str(mail.outbox[0].subject), "[Testing campaign 2019] Nezaplacená faktura"
        )
    ...
    def test_english_notification(self):
        ...
        self.assertEqual(msg.recipients(), ["test@test.cz"])
        self.assertEqual(
            str(mail.outbox[0].subject), "[Testing campaign 2019] Unpaid Invoice"
        )
