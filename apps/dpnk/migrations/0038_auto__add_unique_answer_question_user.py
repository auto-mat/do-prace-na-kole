...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding unique constraint on 'Question', fields ['order', 'competition']
        db.create_unique(u'dpnk_question', ['order', 'competition_id'])

        # Adding unique constraint on 'Answer', fields ['question', 'user']
        db.create_unique(u'dpnk_answer', ['question_id', 'user_id'])


        # Changing field 'Choice.text'
        db.alter_column(u'dpnk_choice', 'text', self.gf('django.db.models.fields.CharField')(max_length=250))
        # Adding index on 'Choice', fields ['text']
        db.create_index(u'dpnk_choice', ['text'])

        # Adding unique constraint on 'Choice', fields ['text', 'choice_type']
        db.create_unique(u'dpnk_choice', ['text', 'choice_type_id'])


    def backwards(self, orm):
        # Removing unique constraint on 'Choice', fields ['text', 'choice_type']
        db.delete_unique(u'dpnk_choice', ['text', 'choice_type_id'])

        # Removing index on 'Choice', fields ['text']
        db.delete_index(u'dpnk_choice', ['text'])

        # Removing unique constraint on 'Answer', fields ['question', 'user']
        db.delete_unique(u'dpnk_answer', ['question_id', 'user_id'])

        # Removing unique constraint on 'Question', fields ['order', 'competition']
        db.delete_unique(u'dpnk_question', ['order', 'competition_id'])


        # Changing field 'Choice.text'
        db.alter_column(u'dpnk_choice', 'text', self.gf('django.db.models.fields.CharField')(max_length=300))
