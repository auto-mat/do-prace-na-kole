...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        campaign = orm['dpnk.Campaign'].objects.get(slug="dpnk2013")

        for city in orm['dpnk.City'].objects.all():
            city_in_campaign = orm['dpnk.CityInCampaign'](
                city = city,
                campaign = campaign)
            city_in_campaign.save()

        for subsidiary in orm['dpnk.Subsidiary'].objects.all():
            subsidiary.city_in_campaign = orm['dpnk.CityInCampaign'].objects.get(campaign=campaign, city=subsidiary.city)
            subsidiary.save()
