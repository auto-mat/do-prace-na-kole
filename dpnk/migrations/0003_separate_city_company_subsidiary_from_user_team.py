...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        liberec = orm['dpnk.City']
        liberec = City(name="Liberec")
        liberec.save()
        
        mesta = { 'Praha' : praha, 'Brno': brno, 'Liberec': liberec }

        for team in Team.objects.all():
            company = orm['dpnk.Company']
            company, creted = Company.objects.get_or_create(name=team.company)
            company.save()

            subsidiary = orm['dpnk.Subsidiary']
            subsidiary, creted = Subsidiary.objects.get_or_create(address=team.address, company = company, city = mesta[team.city])
            subsidiary.save()
