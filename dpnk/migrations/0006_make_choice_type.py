...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        player = orm['dpnk.Competition'].objects.get(id=1)
        company = orm['dpnk.Competition'].objects.get(id=2)
        competitions = { 'player' : player, 'company': company}

        for idx, question in enumerate(orm['dpnk.Question'].objects.all()):
            print competitions[question.questionaire]
            choice_type = orm['dpnk.ChoiceType'](name="Ch%i" % idx, competition = competitions[question.questionaire])
            choice_type.save()
            for choice in orm['dpnk.Choice'].objects.filter(question = question):
                print "     ", choice.text, ", ", choice_type.id
                choice.choice_type = choice_type
                choice.save()
            question.choice_type = choice_type
            question.save()
