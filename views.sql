...
create or replace view dpnk_v_team_results as select t.id as id, t.name as name, t.company as company, t.city as city, sum(p.trips) as trips, sum(p.trips)/count(p.id) as trips_per_person, sum(p.trips*p.distance) as distance from dpnk_userprofile as p left join dpnk_team as t on p.team_id = t.id where p.active=true group by team_id order by trips_per_person desc, distance desc;
